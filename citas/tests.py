...
class CitaTests(TestCase):
    ...
    def test_profesor_puede_crear_horario(self):
        ...
        response = self.client_profe.post(
            '/api/horarios/', {'fecha_hora_inicio': fecha.isoformat()})
        self.assertEqual(response.status_code, 201)

    def test_alumno_no_puede_crear_horario(self):
        fecha = timezone.now() + timedelta(days=1)
        response = self.client_alumno.post(
            '/api/horarios/', {'fecha_hora_inicio': fecha.isoformat()})
    ...
    def test_alumno_puede_reservar_clase(self):
        ...
        horario = HorarioDisponible.objects.create(
            profesor=self.profesor, fecha_hora_inicio=fecha)

        # Alumno reserva ese horario
        response = self.client_alumno.post(
            '/api/clases/', {'horario': horario.id, 'alumno': self.user_alumno.id})
        self.assertEqual(response.status_code, 201)
    ...
    def test_alumno_no_puede_reservar_horario_ocupado(self):
        ...
        otro_alumno = User.objects.create_user(
            username='otro', password='1234')
        client_otro = APIClient()
        client_otro.force_authenticate(user=otro_alumno)
        response = client_otro.post(
            '/api/clases/', {'horario': horario.id, 'alumno': otro_alumno.id})
